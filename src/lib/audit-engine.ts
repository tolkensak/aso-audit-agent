...
function scoreTitle(app: AppMetadata): {
    ...
    if (titleLength <= 30) {
        ...
        recommendations.push({
            ...
            description:
                "Apple enforces a 30-character limit for titles. Your current title exceeds this limit.",
...
function scoreSubtitle(app: AppMetadata): {
    ...
    if (!app.subtitle) {
        ...
        recommendations.push({
            ...
            description:
                "Use the 30 characters to highlight key benefits and secondary keywords.",
...
function scoreKeywordField(app: AppMetadata): {
    score: number;
    checks: string[];
    recommendations: Recommendation[];
} {
    ...
    if (!app.keywordField) {
        ...
        recommendations.push({
            ...
            description:
                "The keyword field is crucial for ASO. Use all 100 characters.",
            evidence: "Keyword field is empty",
            afterExample:
                "music,streaming,podcast,audio,playlists,songs,discover",
...
function scoreDescription(app: AppMetadata): {
    score: number;
    checks: string[];
    recommendations: Recommendation[];
} {
...
function scoreScreenshots(app: AppMetadata): {
    score: number;
    checks: string[];
    recommendations: Recommendation[];
} {
    ...
    if (screenshotCount >= 8) {
        ...
        recommendations.push({
            ...
            description:
                "Apple allows up to 10 screenshots. Use all slots to showcase your app.",
...
function scoreRatings(app: AppMetadata): {
    score: number;
    checks: string[];
    recommendations: Recommendation[];
} {
...
function scoreIcon(app: AppMetadata): {
    score: number;
    checks: string[];
    recommendations: Recommendation[];
} {
    ...
    recommendations.push({
        ...
        title: "Add app icon",
        description:
            "A distinctive icon is crucial for brand recognition in search results.",
...
function scorePromotionalText(app: AppMetadata): {
    score: number;
    checks: string[];
    recommendations: Recommendation[];
} {
    ...
    if (app.promotionalText) {
        const textLength = app.promotionalText.length;
        if (textLength > 50) {
            checks.push(`✅ Promotional text: ${textLength} characters`);
            score += 5;
        } else {
            checks.push("⚠️ Promotional text too short");
            score += 2;
        }

        // Check for keywords and CTAs
        const hasCTA = ["download", "try", "start", "get", "free"].some((w) =>
            app.promotionalText!.toLowerCase().includes(w),
        );
        if (hasCTA) {
            checks.push("✅ Contains call-to-action");
            score += 3;
        }
    } else {
        checks.push("⚠️ No promotional text found");
        recommendations.push({
            type: "quick-win",
            priority: 3,
            title: "Add promotional text",
            description:
                "Use the promotional text field to highlight limited-time offers or key features.",
            evidence: "Promotional text field is empty",
            afterExample: "Try 3 months free. Cancel anytime.",
            impact: "Medium - Boosts conversion",
            effort: "low",
        });
    }
...
function scoreVideo(app: AppMetadata): {
    score: number;
    checks: string[];
    recommendations: Recommendation[];
} {
    ...
    if (app.previewVideo) {
        checks.push("✅ App preview video present");
        return {
            score: 8,
            checks,
            recommendations: [
                {
                    type: "strategic",
                    priority: 1,
                    title: "Optimize app preview video",
                    description:
                        "Ensure the first 3 seconds hook viewers and the video works without sound.",
                    evidence: "Video exists but needs optimization",
    ...
    checks.push("❌ No preview video found");
    recommendations.push({
        type: "high-impact",
        priority: 2,
        title: "Add an app preview video",
        description:
            "A 15-30 second preview video significantly boosts conversion rates.",
        evidence: "No preview video in listing",
        impact: "High - Increases downloads",
        effort: "medium",
    });
...
export function performAudit(appMetadata: AppMetadata): AuditResult {
    ...
    const promoAudit = scorePromotionalText(appMetadata);
    const videoAudit = scoreVideo(appMetadata);
    ...
    const auditData = [
        ...
        { name: "Promotional Text", audit: promoAudit, weight: 5 },
        { name: "Preview Video", audit: videoAudit, weight: 5 },
    ];
